...
- **Show log and report** — every request and message that finishes (cleared by the desk, read by the performer, or cancelled) is kept in a show log, well beyond the board's short *Recently done* list. The **History** tab on the admin page shows it grouped by performer with the time asked, what it was, and how long it took to clear, with a per-performer filter and totals; it updates live. **Download CSV** gives a spreadsheet of the night (`FixMyMix-<show>-<date>.csv`, one row per item, times in UTC), and **Printable report** opens a paper-friendly page with totals and a table per performer — use the browser's Print to save it as a PDF. **Clear history** wipes the log (download first if you want to keep it). The log lives in `data/history.json`, capped at 5000 entries.
